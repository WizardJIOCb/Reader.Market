...
export async function registerRoutes(
  ...
  io.on('connection', (socket) => {
    ...
    if (isAuthenticated) {
      console.log('\x1b[34m%s\x1b[0m', `[WEBSOCKET] рџ”— User ${userId} connected via WebSocket`);
      
      // Join user's personal room for notifications (authenticated only)
      const personalRoom = `user:${userId}`;
      socket.join(personalRoom);
      console.log('\x1b[32m%s\x1b[0m', `[WEBSOCKET] вњ… User ${userId} joined personal room: ${personalRoom}`);
    } else {
      console.log('\x1b[34m%s\x1b[0m', `[WEBSOCKET] рџ”— Unauthenticated user connected via WebSocket`);
    }
    
    // Log all rooms this socket is in
    const rooms = Array.from(socket.rooms);
    console.log('\x1b[36m%s\x1b[0m', `[WEBSOCKET] рџ“‹ Socket rooms: ${JSON.stringify(rooms)}`);
    ...
    socket.on('join:stream:global', () => {
      ...
      console.log('\x1b[32m%s\x1b[0m', `[WEBSOCKET] вњ… Joined global stream room`);
    });
    ...
    socket.on('join:stream:shelves', () => {
      ...
      console.log('\x1b[32m%s\x1b[0m', `[WEBSOCKET] вњ… User ${userId} joined shelves stream room`);
    });
    ...
    socket.on('join:stream:last-actions', () => {
      ...
      console.log('\x1b[32m%s\x1b[0m', `[WEBSOCKET] вњ… Joined last actions stream room`);
    });
    ...
    socket.on('join:book-chat', (bookId: string) => {
      ...
      console.log('\x1b[32m%s\x1b[0m', `[WEBSOCKET] вњ… User ${userId} joined book chat room: ${roomName}`);
...
<head>
    ...
    <style>
        ...
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: var(--background); color: var(--foreground); margin: 0; padding: 20px; }
        .container { max-width: 800px; margin: 0 auto; }
        .message { background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 30px; text-align: center; margin-top: 50px; }
        .success { color: #10b981; font-size: 3rem; margin-bottom: 20px; }
        h1 { color: var(--primary); margin: 0 0 15px 0; }
...
<body>
    <div class="container">
        <div class="message">
            <div class="success">вњ“</div>
            <h1>РљСЌС€ РѕС‡РёС‰РµРЅ</h1>
            <p>РљСЌС€ РёСЃС‚РѕСЂРёРё РєРѕРјРјРёС‚РѕРІ СѓСЃРїРµС€РЅРѕ РѕС‡РёС‰РµРЅ.<br>РџСЂРё СЃР»РµРґСѓСЋС‰РµРј Р·Р°РїСЂРѕСЃРµ РґР°РЅРЅС‹Рµ Р±СѓРґСѓС‚ Р·Р°РіСЂСѓР¶РµРЅС‹ СЃ GitHub.</p>
        </div>
        <div class="footer">Generated by git-to-gpt endpoint вЂў ${new Date().toISOString()}</div>
    </div>
</body>
</html>`;
      ...
      // Check cache first
      const now = Date.now();
      if (commitsCache.data.length > 0 && (now - commitsCache.timestamp) < commitsCache.ttl) {
        console.log(`Using cached commits. Cache age: ${Math.floor((now - commitsCache.timestamp) / 1000)} seconds`);
        
        // Apply count limit if specified
        const cachedCommits = count !== undefined && count > 0 ? commitsCache.data.slice(0, count) : commitsCache.data;
...
<body>
    <div class="container">
        <div class="header">
            <h1>рџ“љ Reader.Market Commits</h1>
            <p>РџРѕР»РЅР°СЏ РёСЃС‚РѕСЂРёСЏ РёР·РјРµРЅРµРЅРёР№ РїСЂРѕРµРєС‚Р°</p>
            <div class="stats">
                <div class="stat-box">Р’СЃРµРіРѕ РєРѕРјРјРёС‚РѕРІ: ${cachedCommits.length}</div>
                <div class="stat-box">Р РµРїРѕР·РёС‚РѕСЂРёР№: WizardJIOCb/Reader.Market</div>
                <div class="stat-box">РћР±РЅРѕРІР»РµРЅРѕ: ${new Date().toLocaleString('ru-RU')}</div>
            </div>
            <div class="cache-info">
                в„№пёЏ Р”Р°РЅРЅС‹Рµ РёР· РєСЌС€Р° (РѕР±РЅРѕРІР»РµРЅС‹ ${(now - commitsCache.timestamp) / 1000} СЃРµРєСѓРЅРґ РЅР°Р·Р°Рґ)
            </div>
        </div>
        
        <div class="commits-grid">
            ${cachedCommits.map((commit: any) => `
                <div class="commit-card">
                    ...
                    <div class="commit-body">
                        <div class="commit-body-content">${commit.body.trim().replace(/      // Process cover image if provided
      let coverImageUrl = null;
      if (req.file) {
        // Move file to collections folder if it's not already there
        const filePath = req.file.path;
        const destDir = path.join(process.cwd(), 'uploads', 'collections');
        ...
        // Move the file to the collections directory
        fs.renameSync(filePath, destPath);
        
        coverImageUrl = /uploads/collections/;
      }
/g, '<br>')}</div>
                    </div>
                    ` : ''}
                    ...
                    <a href="${commit.url}" target="_blank" class="commit-link">
                        рџ“„ РџРѕСЃРјРѕС‚СЂРµС‚СЊ РЅР° GitHub
                    </a>
                </div>
            `).join('')}
        </div>
        ...
        <div class="footer">
            Generated by git-to-gpt endpoint вЂў Cached response вЂў ${new Date().toISOString()}
        </div>
    </div>
</body>
</html>`;
        ...
        // Return JSON from cached data
        return res.json({
          success: true,
          repository: 'WizardJIOCb/Reader.Market',
          url: `https://github.com/WizardJIOCb/Reader.Market/commits/main`,
          ...
          total_fetched: cachedCommits.length,
      ...
      while (hasNextPage && (count === 0 || commits.length < (count || 100))) {
        ...
        const pageCommits = commitsData.map((commit: any) => {
          ...
          const messageLines = fullMessage.split('      // Process cover image if provided
      let coverImageUrl = null;
      if (req.file) {
        // Move file to collections folder if it's not already there
        const filePath = req.file.path;
...
');
          ...
          const body = messageLines.slice(1).join('      // Process cover image if provided
      let coverImageUrl = null;
      if (req.file) {
        // Move file to collections folder if it's not already there
        const filePath = req.file.path;
...
').trim();
      ...
      if (template === 'cool') {
        ...
        activityGraph += '<div class="graph-header"><h3>РђРєС‚РёРІРЅРѕСЃС‚СЊ РєРѕРјРјРёС‚РѕРІ Р·Р° РіРѕРґ</h3></div>';
        activityGraph += '<div class="graph-container">';
        
        // Weekday labels
        activityGraph += '<div class="weekday-labels">';
        activityGraph += '<div></div>'; // Empty corner
        ['РџРЅ', 'РЎСЂ', 'РџС‚'].forEach(day => {
        ...
        dates.forEach(day => {
            ...
            activityGraph += `<div class="activity-cell" 
                               ...
                               title="${day.date}: ${day.count} РєРѕРјРјРёС‚РѕРІ"></div>`;
        });
        activityGraph += '</div>';
        
        // Legend
        activityGraph += '<div class="graph-legend">';
        activityGraph += '<span>РњРµРЅСЊС€Рµ</span>';
        ...
        activityGraph += '</div>';
        activityGraph += '<span>Р‘РѕР»СЊС€Рµ</span>';
        activityGraph += '</div>';
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>РџРѕР»РЅР°СЏ РёСЃС‚РѕСЂРёСЏ РёР·РјРµРЅРµРЅРёР№ РїСЂРѕРµРєС‚Р°</p>
            <div class="stats">
                <div class="stat-box">Р’СЃРµРіРѕ РєРѕРјРјРёС‚РѕРІ: ${finalCommits.length}</div>
                <div class="stat-box">Р РµРїРѕР·РёС‚РѕСЂРёР№: WizardJIOCb/Reader.Market</div>
                <div class="stat-box">РћР±РЅРѕРІР»РµРЅРѕ: ${new Date().toLocaleString('ru-RU')}</div>
            </div>
        </div>
...
/g, '<br>')}</div>
        ...
        <div class="footer">
            Generated by git-to-gpt endpoint вЂў ${new Date().toISOString()}
        </div>
    </div>
</body>
</html>`;
    ...
    try {
      ...
      if (!skipCache && apiGitCache.data && (now - apiGitCache.timestamp) < apiGitCache.ttl) {
        ...
        if (!cachedHtml.includes('<!-- Cache updated:')) {
          cachedHtml = cachedHtml.replace('</body>', `${timestampComment}      // Process cover image if provided
      let coverImageUrl = null;
      if (req.file) {
        // Move file to collections folder if it's not already there
        const filePath = req.file.path;
      ...
      if (!html.includes('<!-- Cache updated:')) {
        modifiedHtml = html.replace('</body>', `${timestampComment}      // Process cover image if provided
      let coverImageUrl = null;
      if (req.file) {
        // Move file to collections folder if it's not already there
        const filePath = req.file.path;
  ...
  app.post("/api/auth/register", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.log('[Registration] вњ… Default shelf created');
      } catch (shelfError) {
      ...
      try {
        ...
        if ((app as any).io && action) {
          ...
          console.log('[Registration] вњ… Registration event broadcasted to stream:global and stream:last-actions');
        }
      } catch (actionError) {
    ...
    try {
      ...
      try {
        ...
        if (user && newsItem && (app as any).io) {
          ...
          console.log('\x1b[32m%s\x1b[0m', '[STREAM] вњ… News comment broadcast sent to stream:global');
      ...
      if (published) {
        try {
          ...
          if (user && (app as any).io) {
            ...
            // Broadcast to global stream
            io.to('stream:global').emit('stream:new-activity', activityData);
            console.log('\x1b[32m%s\x1b[0m', '[STREAM DEBUG] вњ… Direct broadcast sent to stream:global');
          } else {
    ...
    try {
      ...
      try {
        if ((app as any).io) {
          ...
          console.log('\x1b[32m%s\x1b[0m', '[STREAM] вњ… Deletion broadcast sent');
        }
      } catch (streamError) {
  ...
  app.post("/api/articles/:id/like", authenticateToken, async (req, res) => {
    ...
    try {
      const { id } = req.params;
      const userId = (req as any).user.userId;
      const emoji = req.body.Emoji || req.body.emoji || 'рџ‘Ќ';
  ...
  app.post("/api/comments/:id/like", authenticateToken, async (req, res) => {
    ...
    try {
      ...
      const userId = (req as any).user.userId;
      const emoji = req.body.emoji || 'рџ‘Ќ';
      ...
      if (query && query.trim() && userId && process.env.ENABLE_LAST_ACTIONS_TRACKING === 'true') {
        try {
          ...
          if ((req.app as any).io && action) {
            ...
            io.to('stream:last-actions').emit('stream:last-action', eventData);
            console.log('[Search] вњ… Search event broadcasted');
          }
        } catch (actionError) {
  ...
  app.post("/api/bookmark-collections", authenticateToken, upload.single('coverImage'), async (req, res) => {
    try {
      const userId = (req as any).user.userId;
      
      // Handle both regular form data and file uploads
      ...
      // Parse bookIds if they are sent as arrays in form data
      let parsedBookIds = [];
      if (bookIds) {
        if (typeof bookIds === 'string') {
          // If it's a single string, convert to array
          parsedBookIds = [bookIds];
      ...
      const collection = await storage.createBookmarkCollection({
        ...
        bookIds: parsedBookIds // Include bookIds array if provided
      });
  ...
  app.put("/api/bookmark-collections/:id", authenticateToken, upload.single('coverImage'), async (req, res) => {
    try {
      const { id } = req.params;
      const userId = (req as any).user.userId;
      ...
      // Parse bookIds if they are sent as arrays in form data
      if (updateData.bookIds) {
        if (typeof updateData.bookIds === 'string') {
          // If it's a single string, convert to array
          updateData.bookIds = [updateData.bookIds];
        } else if (Array.isArray(updateData.bookIds)) {
          // If it's already an array, use as is
          updateData.bookIds = updateData.bookIds;
        }
      }
  ...
  app.post("/api/bookmark-collections/:id/clone", authenticateToken, async (req, res) => {
    try {
      ...
      const newCollectionData = {
        userId,
        name: name || `РљРѕРїРёСЏ ${collectionToClone.name}`,
  ...
  app.get("/api/bookmark-collections/book/:bookId", authenticateToken, async (req, res) => {
    try {
      ...
      const collectionsWithCounts = await Promise.all(uniqueCollections.map(async (collection) => {
        ...
        const isClone = collection.name.startsWith('РљРѕРїРёСЏ ');
  ...
  app.post("/api/books/upload", authenticateToken, upload.fields([{ name: 'bookFile' }, { name: 'coverImage' }]), async (req, res) => {
    ...
    try {
      ...
      let uploadedShelf = (await storage.getShelves(userId)).find(shelf => shelf.name === "Р—Р°РіСЂСѓР¶РµРЅРЅС‹Рµ");
      
      if (!uploadedShelf) {
        // Create the "Р—Р°РіСЂСѓР¶РµРЅРЅС‹Рµ" shelf
        uploadedShelf = await storage.createShelf(userId, {
          name: "Р—Р°РіСЂСѓР¶РµРЅРЅС‹Рµ",
          description: "Р—Р°РіСЂСѓР¶РµРЅРЅС‹Рµ РєРЅРёРіРё",
          color: "bg-blue-100 dark:bg-blue-900/20"
        });
      }
    ...
    try {
      ...
      try {
        ...
        if ((app as any).io && action) {
          ...
          io.to('stream:global').emit('stream:last-action', eventData);
          io.to('stream:last-actions').emit('stream:last-action', eventData);
          console.log('[Shelf Creation] вњ… Shelf creation event broadcasted');
        }
      } catch (actionError) {
    ...
    try {
      ...
      try {
        ...
        if ((app as any).io && action) {
          ...
          io.to('stream:global').emit('stream:last-action', eventData);
          io.to('stream:last-actions').emit('stream:last-action', eventData);
          console.log('[Book to Shelf] вњ… Book added to shelf event broadcasted');
        }
      } catch (actionError) {
          ...
          if (process.env.ENABLE_LAST_ACTIONS_TRACKING === 'true') {
            ...
            if (comment) {
              ...
              if ((app as any).io && userAction) {
                ...
                io.to('stream:last-actions').emit('stream:last-action', eventData);
                console.log('[Book Comment Reaction] вњ… Broadcast sent');
              }
            }
          }
        } catch (actionError) {
          ...
          if (process.env.ENABLE_LAST_ACTIONS_TRACKING === 'true') {
            ...
            if (review) {
              ...
              if ((app as any).io && userAction) {
                ...
                io.to('stream:last-actions').emit('stream:last-action', eventData);
                console.log('[Book Review Reaction] вњ… Broadcast sent');
              }
            }
          }
        } catch (actionError) {
  ...
  */
  ...
  app.post("/api/admin/books", authenticateToken, requireAdminOrModerator, async (req, res) => {
    ...
    try {
      ...
      let externalShelf = (await storage.getShelves((req as any).user.userId)).find(shelf => shelf.name === "Р’РЅРµС€РЅРёРµ РёСЃС‚РѕС‡РЅРёРєРё");
      
      if (!externalShelf) {
        // Create the "Р’РЅРµС€РЅРёРµ РёСЃС‚РѕС‡РЅРёРєРё" shelf
        externalShelf = await storage.createShelf((req as any).user.userId, {
          name: "Р’РЅРµС€РЅРёРµ РёСЃС‚РѕС‡РЅРёРєРё",
          description: "РљРЅРёРіРё, РґРѕР±Р°РІР»РµРЅРЅС‹Рµ РёР· РІРЅРµС€РЅРёС… РёСЃС‚РѕС‡РЅРёРєРѕРІ",
  ...
  app.post("/api/messages", authenticateToken, async (req, res) => {
    ...
    try {
      ...
      let attachmentMetadata = null;
      if (attachments && Array.isArray(attachments) && attachments.length > 0) {
        console.log('рџ”µ Processing attachments:', attachments);
        const uploadedAttachments = [];
        for (const uploadId of attachments) {
          const fileUpload = await storage.getFileUpload(uploadId);
          console.log('рџ”µ File upload for', uploadId, ':', fileUpload);
          if (fileUpload && fileUpload.uploaderId === userId && fileUpload.entityType === 'temp') {
            uploadedAttachments.push({
              url: fileUpload.fileUrl,
              filename: fileUpload.filename,
        ...
        if (uploadedAttachments.length > 0) {
          attachmentMetadata = { attachments: uploadedAttachments };
          console.log('рџџў Created attachmentMetadata:', JSON.stringify(attachmentMetadata, null, 2));
        }
      }
      ...
      console.log('рџџЎ Calling createMessage with data:', JSON.stringify(messageData, null, 2));
      
      // Create message with attachments
      const message = await storage.createMessage(messageData);
      console.log('рџџ  createMessage returned:', JSON.stringify(message, null, 2));
      
      // Log message structure for debugging
      console.log('рџ”µ Message structure keys:', Object.keys(message));
      console.log('рџ”µ Message sender data:', {
      ...
      console.log('рџџў Enriched message for WebSocket:', {
      ...
      const io = (app as any).io;
      if (io) {
        console.log('\x1b[35m%s\x1b[0m', '[WEBSOCKET] рџ“Ў Emitting WebSocket events for new message');
        ...
        console.log('\x1b[32m%s\x1b[0m', `[WEBSOCKET] вњ… Emitting 'notification:new' to room: ${recipientRoom}`);
        
        // ALSO send message:new to recipient's personal room for notifications
        console.log('\x1b[36m%s\x1b[0m', `[WEBSOCKET] рџ“© ALSO emitting 'message:new' to recipient room: ${recipientRoom}`);
        ...
        console.log('\x1b[33m%s\x1b[0m', `[WEBSOCKET] рџ‘Ґ Number of clients in room '${recipientRoom}': ${sockets.length}`);
        if (sockets.length === 0) {
          console.log('\x1b[31m%s\x1b[0m', `[WEBSOCKET] вљ пёЏ  WARNING: No clients connected to room '${recipientRoom}'!`);
        } else {
          console.log('\x1b[32m%s\x1b[0m', `[WEBSOCKET] вњ… Event sent to ${sockets.length} client(s)`);
        }
      } else {
        console.log('\x1b[31m%s\x1b[0m', '[WEBSOCKET] вќЊ ERROR: Socket.IO instance not found!');
      }
  ...
  app.get("/api/conversations", authenticateToken, async (req, res) => {
    ...
    console.log("      // Process cover image if provided
      let coverImageUrl = null;
      if (req.file) {
        // Move file to collections folder if it's not already there
        const filePath = req.file.path;
...
=== GET /api/conversations ===");
    ...
    try {
      ...
      if (conversations.length > 0) {
        ...
        console.log("вљ пёЏ  WARNING: No conversations found for this user!");
      }
      console.log("=========================      // Process cover image if provided
      let coverImageUrl = null;
      if (req.file) {
...
");
      ...
      console.error("вќЊ Get conversations error:", error);
  ...
  app.post("/api/groups/:groupId/members", authenticateToken, async (req, res) => {
    ...
    try {
      ...
      if (!newMemberId) {
        ...
        return res.status(400).json({ error: "РќРµ СѓРєР°Р·Р°РЅ ID РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ" });
      }
      ...
      if (role !== 'administrator' && role !== 'moderator') {
        ...
        return res.status(403).json({ error: "РќРµРґРѕСЃС‚Р°С‚РѕС‡РЅРѕ РїСЂР°РІ РґР»СЏ РґРѕР±Р°РІР»РµРЅРёСЏ СѓС‡Р°СЃС‚РЅРёРєРѕРІ" });
      }
      ...
      if (isMember) {
        return res.status(400).json({ error: "РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ СѓР¶Рµ РІ РіСЂСѓРїРїРµ" });
      }
      ...
      res.status(500).json({ error: "РќРµ СѓРґР°Р»РѕСЃСЊ РґРѕР±Р°РІРёС‚СЊ СѓС‡Р°СЃС‚РЅРёРєР°" });
    }
  });
      ...
      try {
        if (process.env.ENABLE_LAST_ACTIONS_TRACKING === 'true') {
          ...
          if ((app as any).io && action) {
            ...
            // Broadcast to last-actions room
            io.to('stream:last-actions').emit('stream:last-action', eventData);
            console.log('[Profile Rating] вњ… Profile rating event broadcasted');
          }
        }
      } catch (actionError) {
      ...
      try {
        ...
        if (process.env.ENABLE_LAST_ACTIONS_TRACKING === 'true') {
          ...
          if ((app as any).io && action) {
            ...
            // Broadcast to last-actions room
            io.to('stream:last-actions').emit('stream:last-action', eventData);
            console.log('[Profile Comment] вњ… Profile comment event broadcasted');
...
\n    }
  ...
  app.get("/api/admin/collections", authenticateToken, requireAdminOrModerator, async (req, res) => {
    try {
      const collections = await db.select({
        ...
        bookmarkCount: sql<number>`COALESCE((SELECT COUNT(*) FROM ${bookmarkCollectionItems} WHERE ${bookmarkCollectionItems.collectionId} = ${bookmarkCollections.id}), 0)`.mapWith(Number)
      })
      .from(bookmarkCollections)
      .leftJoin(users, eq(bookmarkCollections.userId, users.id))
      .orderBy(desc(bookmarkCollections.createdAt));
      
      // Add owner information to each collection
      const collectionsWithOwners = await Promise.all(collections.map(async (collection) => {
        // Get user info for the collection owner
        const user = await db.select({
          username: users.username,
          fullName: users.fullName
        }).from(users).where(eq(users.id, collection.userId)).limit(1);
        
        return {
          ...collection,
          ownerUsername: user[0]?.username,
          ownerFullName: user[0]?.fullName
        };
      }));
      
      res.json(collectionsWithOwners);
    } catch (error) {
  ...
  app.post("/api/admin/collections", authenticateToken, requireAdminOrModerator, upload.single('coverImage'), async (req, res) => {
    try {
  ...
  app.put("/api/admin/collections/:id", authenticateToken, requireAdminOrModerator, upload.single('coverImage'), async (req, res) => {
    try {
      const { id } = req.params;
      const { name, description, color, isPublic, userId } = req.body;
  ...
  app.delete("/api/admin/collections/:id", authenticateToken, requireAdminOrModerator, async (req, res) => {
    try {
      const { id } = req.params;
